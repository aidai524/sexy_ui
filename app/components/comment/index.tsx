...
import { formatAddress, formatDateTime, httpGet, httpAuthPost, httpAuthDelete } from '@/app/utils'
...
export default function CommentComp({
    id, showEdit = true, usePanel = true
}: Props) {
        ...
        return <div key={item.id} className={styles.comment}>
            ...
            <div className={styles.commentAction}>
                ...
                <div onClick={async () => {
                    ...
                    <div>
                        {
                            item.isUnlike ? <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M10.1655 9.59131H14.4261C14.4261 9.59131 15.4967 9.68021 15.4967 8.56758C15.4967 8.56758 15.4967 6.96525 13.9129 1.58C13.9129 1.58 13.4223 2.00272e-05 12.8199 2.00272e-05H6.41836C5.97214 2.00272e-05 4.83464 0.300641 4.83464 1.13532V9.59131C4.83464 9.59131 8.00179 11.9279 8.00179 14.9097C8.02017 14.9417 8.02441 15.0067 8.02982 15.0897C8.05074 15.4103 8.08921 16 9.02801 16C9.02801 16 11.8164 15.6886 10.1655 9.59131ZM0.534994 9.5885H3.08621C3.08621 9.5885 3.73464 9.59678 3.73464 8.94179V0.359091C3.73464 0.359091 3.73871 2.86102e-05 3.2787 2.86102e-05H1.07061C1.07061 2.86102e-05 0.526875 2.95639e-05 0.526875 0.54283L0 9.071C0 9.071 0.0081184 9.5885 0.534994 9.5885Z" fill="#6EAACA" />
                            </svg> : <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    return <div className={styles.main}>
        ...
        {
            showEdit && <input value={commentText} onKeyUp={async (e) => {
                if (e.code === 'Enter') {
                    const query: any = {
                        project_id: id,
                        text: commentText
        ...
        {
            commentList.length > 0 && (usePanel ?
                <Panel>
                    {CommentList}
                </Panel> : <div style={{ padding: '0 10px' }}>
                    {CommentList}
                </div>
            )
        }
