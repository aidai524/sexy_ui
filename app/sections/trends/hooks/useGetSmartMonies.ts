...
export const useGetSmartMonies = ({chain, page, pageSize}: {chain: string, page: number, pageSize: number}) => {
  ...
  const fetchSmartMonies = async () => {
    ...
    const res = await copyTradeService.getSmartMonies({ chain, page, pageSize });
