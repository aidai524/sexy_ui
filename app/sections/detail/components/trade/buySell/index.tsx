        ...
        <div className={[styles.cationArea, styles.panel].join(' ')}>
            ...
            <div className={styles.inputArea}>
                <div className={styles.actionArea}>
                    {
                        activeIndex === 0 ? <div className={styles.switchToken} onClick={() => {
                            if (tokenType === 0) {
                                setCurrentToken(SOL)
                                setTokenType(1)
                                setValInput('')
                ...
                {
                    activeIndex === 0 && (tokenType === 1 ? <div className={styles.tokenPercent}>
                        <div onClick={() => {
                            setSolPercent(0);
                            setValInput('')
                    ...
                    </div> : <div className={styles.paid}>
                        <div>You will paid by</div>
                        <div>{buyInSol && new Big(buyInSol).div(10 ** SOL.tokenDecimals).toFixed()} SOL</div>
                    </div>)
                }
                            ...
                            TOKEN_PERCENT_LIST.map((item) => {
                                return <div
                                    onClick={() => {
                                        ...
                                        const tokenPercentVal = new Big(tokenBalance).mul(item / 100).toFixed(2)
                                        console.log('item:', item, tokenBalance, tokenPercentVal)
                                        setValInput(tokenPercentVal)
                                    }}
                                    key={item} className={[styles.percentTag, item === tokenPercent ? styles.tagActive : ''].join(' ')}>{item}%</div>
                            })
                        }
                    </div>
                }
